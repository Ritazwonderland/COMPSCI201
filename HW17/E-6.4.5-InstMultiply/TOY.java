    ...
    public void run() {
        while (true) {
            ...
            switch (op) {
                ...
                case  2: 
                    /*reg[d]=((short)reg[s]*(short)reg[t])>>16;
                    if (d+1>=16) reg[0]=(reg[s]*reg[t])&0xFFFF;
                    else reg[(d+1)]=(reg[s]*reg[t])&0xFFFF;
                    break;// subtract to multi;*/
                    int a = (short) reg[s] * (short) reg[t]; reg[d] = (a >> 16) & 0xFFFF; reg[(d+1)%16] = a & 0xFFFF; break;
