...
/*Develop versions of stdin() and stdout() for TOY.java 
that simulate punched paper tape: for each 16-bit value, 
use two 8-character lines, with a - corresponding to each 0 bit 
and a * corresponding to each 1 bit. */
...
public class TOY { 
    ...
    public void run() {
        ...
        while (true) {
            ...
            // stdin, to be modified
            if ((addr == 255 && op == 8) || (reg[t] == 255 && op == 10))
                mem[255] = Integer.parseInt
                (StdIn.readString().replace('-','0').replace('*','1')
                +StdIn.readString().replace('-','0').replace('*','1'),2);
            ...
            // stdout, to be modified
            if ((addr == 255 && op == 9) || (reg[t] == 255 && op == 11))
                //StdOut.println(toHex(mem[255]));
                {
                    String bin=Integer.toBinaryString(mem[255]);
                    String out=("0".repeat(16-bin.length())+bin).replace('0','-').replace('1','*');
                    StdOut.println(out.substring(0,8));
                    StdOut.println(out.substring(8,16));
                }
